...
Die Auswahl der passenden Funktion ist also davon abhängig, was das Ziel ist und in welchem output-Format ihr euer Ergebnis gerne haben würdet. Nachfolgend jeweils ein kurzes Beispiel für jede der vier Funktionen
...
Sind Schleifen also unnötig? Nein. Es gibt sicher einige Situationen, in denen R keine passende Funktion für das habt, was ihr tun wollt. Spezifische Änderungen an einem großen data frame lassen sich z.B. super über Schleifen lösen. Es kann allerdings hilfreich sein, vorher einmal zu schauen, ob es nicht schon passende Alternativen gibt, die schneller und effizienter sind, als die selbst geschriebene Schleife. Wichtig bei der Erstellung von Schleifen ist:
...
-   Das Wichtigste: **Schleifen vermeiden, wenn es bessere Alternativen gibt.** R und alle zugehörigen packages bieten einem unfassbar viele Möglichkeiten. Wer vorher länger sucht, hat hinterher vielleicht weniger Arbeit.
    ...
    ```{r}
    #Für jede Zahl von 1 bis 1000000 soll 15 addiert werden
    loop1 <- function(i){
      for(i in 1:1000000){
      N1 <- i
      N2 <- 15
...
Die Unterschiede werden umso größer, je mehr Wiederholungen man in der Schleife durchführt

```{r}
vergleich <- microbenchmark(times = 100, unit = "s", 
               loop1(10^7), loop2(10^7))
autoplot(vergleich)
```
